...
`+JSON.stringify(o,null,2):l=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:T,correlationId:b,..._}=n;Object.keys(_).length>0&&(u=` {${Object.entries(_).map(([f,R])=>`${f}=${R}`).join(", ")}}`)}let g=`[${i}] [${a}] [${d}] ${c}${s}${u}${l}`;if(this.logFilePath)try{(0,I.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",u={...s,location:l};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},m=new H;var Rt={};function bt(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,ce.fileURLToPath)(Rt.url))}var Ot=bt(),C=y.get("CLAUDE_MEM_DATA_DIR"),W=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,de.homedir)(),".claude"),Ht=(0,h.join)(C,"archives"),Wt=(0,h.join)(C,"logs"),Yt=(0,h.join)(C,"trash"),Vt=(0,h.join)(C,"backups"),qt=(0,h.join)(C,"modes"),Kt=(0,h.join)(C,"settings.json"),me=(0,h.join)(C,"claude-mem.db"),Jt=(0,h.join)(C,"vector-db"),zt=(0,h.join)(W,"settings.json"),Qt=(0,h.join)(W,"commands"),Zt=(0,h.join)(W,"CLAUDE.md");function le(r){(0,pe.mkdirSync)(r,{recursive:!0})}function ue(){return(0,h.join)(Ot,"..")}var $=class{db;constructor(e=me){e!==":memory:"&&le(C),this.db=new _e.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.createRawToolEventsTable()}initializeSchema(){this.db.run(`
    ...
    `),this.db.run("CREATE INDEX IF NOT EXISTS idx_raw_tool_events_session ON raw_tool_events(session_db_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_raw_tool_events_status ON raw_tool_events(status, created_at_epoch)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_raw_tool_events_project ON raw_tool_events(project)"),m.info("DB","Created raw_tool_events table (migration 21)"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(21,new Date().toISOString()),this.migratePendingMessagesToRawEvents())}migratePendingMessagesToRawEvents(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(22))return;if(!this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='pending_messages'").get()){this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(22,new Date().toISOString());return}let s=this.db.run(`
    ...
    `);s.changes>0&&m.info("DB",`Migrated ${s.changes} pending messages to raw_tool_events (migration 22)`),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(22,new Date().toISOString())}updateMemorySessionId(e,t){this.db.prepare(`
      UPDATE sdk_sessions
  ...
  `).all(...e,...s,...o,t.totalObservationCount)}function Re(r,e,t){let s=e.map(()=>"?").join(",");return r.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(...e,t.sessionCount+V)}function Ct(r){return r.replace(/\//g,"-")}function It(r){try{if(!(0,P.existsSync)(r))return{userMessage:"",assistantMessage:""};let e=(0,P.readFileSync)(r,"utf-8").trim();if(!e)return{userMessage:"",assistantMessage:""};let t=e.split(`
...
`}function et(r,e,t,s){let n=[];return s?n.push(...Xe(r)):n.push(...ye(r)),s?n.push(...Ge()):n.push(...ve()),s?n.push(...Be()):n.push(...Ae()),s?n.push(...He()):n.push(...Le()),F(t)&&(s?n.push(...We(e,t)):n.push(...Me(e,t))),n}var ee=A(require("path"),1);function G(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return m.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function st(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function rt(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function nt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function tt(r,e){return ee.default.isAbsolute(r)?ee.default.relative(e,r):r}function ot(r,e,t){let s=G(r);if(s.length>0)return tt(s[0],e);if(t){let n=G(t);if(n.length>0)return tt(n[0],e)}return"General"}function yt(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=nt(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function vt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?G(r.facts).join(`
`):null}function At(r,e,t,s,n,o){let i=[];o?i.push(...Ye(r)):i.push(...De(r));let a=null,d="",c=!1;for(let l of e)if(l.type==="summary"){c&&(i.push(""),c=!1,a=null,d="");let u=l.data,g=st(u.displayTime);o?i.push(...Je(u,g)):i.push(...$e(u,g))}else{let u=l.data,g=ot(u.files_modified,n,u.files_read),E=rt(u.created_at),T=E!==d,b=T?E:"";d=E;let _=t.has(u.id);if(g!==a&&(c&&i.push(""),o?i.push(...Ve(g)):i.push(...xe(g)),a=g,c=!0),_){let S=vt(u,s);o?i.push(...Ke(u,E,T,S,s)):(c&&!o&&(i.push(""),c=!1),i.push(...Ue(u,b,S,s)),a=null)}else o?i.push(qe(u,E,T,s)):i.push(ke(u,b,s))}return c&&i.push(""),i}function it(r,e,t,s,n){let o=[],i=yt(r);for(let[a,d]of i)o.push(...At(a,d,e,t,s,n));return o}function at(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function dt(r,e){let t=[];return e?(t.push(...x("Investigated",r.investigated,p.blue)),t.push(...x("Learned",r.learned,p.yellow)),t.push(...x("Completed",r.completed,p.green)),t.push(...x("Next Steps",r.next_steps,p.magenta))):(t.push(...D("Investigated",r.investigated)),t.push(...D("Learned",r.learned)),t.push(...D("Completed",r.completed)),t.push(...D("Next Steps",r.next_steps))),t}function pt(r,e){return e?ze(r):we(r)}function ct(r,e,t){return!F(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?Qe(r.totalDiscoveryTokens,r.totalReadTokens):Fe(r.totalDiscoveryTokens,r.totalReadTokens)}var Lt=mt.default.join((0,lt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function Mt(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,ut.unlinkSync)(Lt)}catch(e){m.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return m.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function Dt(r,e){return e?Ze(r):Pe(r)}function xt(r,e,t,s,n,o,i){let a=[],d=K(e);a.push(...et(r,d,s,i));let c=t.slice(0,s.sessionCount),l=Ne(c,t),u=Z(e,l),g=Ce(e,s.fullObservationCount);a.push(...it(u,g,s,n,i));let E=t[0],T=e[0];at(s,E,T)&&a.push(...dt(E,i));let b=Q(e,s,o,n);return a.push(...pt(b,i)),a.push(...ct(d,s,i)),a.join(`
